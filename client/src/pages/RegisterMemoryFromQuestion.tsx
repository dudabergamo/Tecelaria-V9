...
export default function RegisterMemoryFromQuestion() {
  const [location, setLocation] = useLocation();
  const { user } = useAuth();
  
  // Extract questionId from URL (required for this page)
  const params = new URLSearchParams(window.location.search);
  const questionId = params.get('questionId') ? parseInt(params.get('questionId')!) : undefined;
  
  console.log('[RegisterMemoryFromQuestion] URL:', location);
  console.log('[RegisterMemoryFromQuestion] questionId:', questionId);
  
  // Redirect to dashboard if no questionId (using useEffect to avoid render issues)
  useEffect(() => {
    if (!questionId) {
      console.log('[RegisterMemoryFromQuestion] No questionId found, redirecting to dashboard');
      setLocation("/dashboard");
    }
  }, [questionId, setLocation]);
  ...
  const { data: questionData, isLoading: isLoadingQuestion } = trpc.questions.getQuestionById.useQuery(
    { id: questionId! },
    { enabled: !!questionId }
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!questionData?.categoryId) {
      toast.error("Erro: categoria da pergunta não encontrada");
      return;
    }
  ...
  // Get category name
  const categoryName = categories.find(c => c.id === questionData?.categoryId)?.name || "Carregando...";
        ...
        <Card className="max-w-md">
          ...
          <CardContent>
            <p className="text-muted-foreground mb-4">
              Não foi possível carregar a pergunta selecionada.
            </p>
            <Button asChild>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="border-b bg-card">
        <div className="container py-4">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold">Registrar Memória de Caixinha</h1>
          </div>
        </div>
      </header>
      ...
      <div className="container py-8 max-w-4xl">
        ...
        <Card className="mb-6 border-2 border-[#8B9D83] bg-gradient-to-br from-[#8B9D83]/10 to-transparent">
          <CardHeader>
            <CardTitle className="text-lg flex items-center gap-2">
              <HelpCircle className="h-5 w-5 text-[#8B9D83]" />
              Pergunta que você está respondendo
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="flex gap-2 flex-wrap">
              <Badge variant="outline" className="bg-[#8B9D83]/10 text-[#8B9D83] border-[#8B9D83]/30">
                Caixinha {questionData.box}
              </Badge>
              <Badge variant="outline" className="bg-[#8B9D83]/10 text-[#8B9D83] border-[#8B9D83]/30">
                Pergunta {questionData.number}
              </Badge>
              <Badge variant="outline" className="bg-[#E07856]/10 text-[#E07856] border-[#E07856]/30">
                {categoryName}
              </Badge>
            </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-4">
            <Button
              ...
              asChild
            >
              <Link href="/caixinhas">Cancelar</Link>
            </Button>
            <Button
              type="submit"
              className="flex-1 bg-[#8B9D83] hover:bg-[#8B9D83]/90"
